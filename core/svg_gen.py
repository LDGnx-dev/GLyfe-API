...
def build_bg_svg(w, h, color):
    ...
    num_cells = random.randint(int((w * h) * 0.20), int((w * h) * 0.60))
    ...
    for (x, y) in seeds:
        ...
        dur = round(random.uniform(3.0, 8.0), 1) # Duración del parpadeo
        delay = round(random.uniform(0.0, 5.0), 1) # Retraso inicial 
